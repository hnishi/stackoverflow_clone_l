...
- 2. `$ yarn eslint`を実行すると下記のエラーが表示されることを確認する。
```
...
- 5. `$ yarn eslint`を実行するとエラーが表示されずに自動的にフォーマットが修正されていることを確認する。
...
* 子のコンポーネントにデータを渡したい場合、[BookDetailPage.vue](../../web/static/pages/BookDetailPage.vue)の`:book="book"`のように`:question="question"`と書くと子のコンポーネントのpropsとして渡せるので、`<question :question="question"/>`と書けばいい。
  ...
  * Vueでのデータの渡し方の詳細は[Vueのページ(Passing Data to Child Components with Props)](https://vuejs.org/v2/guide/components.html#Passing-Data-to-Child-Components-with-Props)を参照。
...
* mochaの`it()`でテストケースを書く。
  * vue-test-utilsの[shallowMount()](https://vue-test-utils.vuejs.org/api/#shallowmount)でコンポーネントをrenderさせる。
